...
### 主键 + RR
![image.png](https://www.hounk.world/upload/2021/02/image-7cfebef647db4a99ad4c816fc595a0db.png)
假设条件是：
...
### 唯一键 + RR
![image.png](https://www.hounk.world/upload/2021/02/image-4bd661e941504a8ba01462aa5f65757e.png)
假设条件是：
* update t1 set name=‘XX’ where id=10。
* id 为唯一索引。
...
### 非唯一键 + RR
![image.png](https://www.hounk.world/upload/2021/02/image-33455b995f8d45a5a0f2ec3066fa9138.png)
假设条件是：
* update t1 set name=‘XX’ where id=10。
* id 为非唯一索引。
...
### 无索引 + RR
![image.png](https://www.hounk.world/upload/2021/02/image-685ecea4b9bf4effa2626bfb97a93bb0.png)
假设条件是：
* update t1 set name=‘XX’ where id=10。
* id 列无索引。
...
### 死锁案例
基于资源争用导致的死锁：
![image.png](https://www.hounk.world/upload/2021/02/image-bd70f00837784630ae1fcea6fc8bb39c.png)
session1 首先拿到 id=1 的锁，session2 同期拿到了 id=5 的锁后，两者分别想拿到对方持有的锁，于是产生死锁。

基于抢占元数据导致的死锁（Metadata lock 即元数据锁）
![image.png](https://www.hounk.world/upload/2021/02/image-3dbb9d96e51b47cb9e1328a97bc37d1c.png)
session1 和 session2 都在抢占 id=1 和 id=6 的元数据的资源，产生死锁。

### 如何解决
查看 MySQL 数据库中死锁的相关信息，可以执行 show engine innodb status 来进行查看，重点关注 “latest detected deadlock” 部分。

避免线上业务因死锁造成的不必要的影响的措施：
